...
// For Computer Graphics Programming, 5th. semester Medialogy, AAU-CPH.
// By: Emil Rose Høeg & Michael Bjørn Nielsen

Shader "Custom/BetterToonShaderMod" {
   Properties {
	...
	float3 texColor =  tex2D(_MainTex, input.uv);
	
	float3 ambientLight = (1-diffuseCutoff) * _UnlitColor.xyz * texColor; //adds general ambient illumination
	float3 diffuseReflection = (1-specularCutoff) * _Color.xyz * diffuseCutoff * texColor;
	float3 specularReflection = _SpecColor.xyz * specularCutoff * texColor;
